...
impl OpenAIContent {
    ...
    fn serialize_as_string(&self) -> serde_json::Value {
        let text_parts: Vec<String> = self.items
            .iter()
            .filter_map(|item| match item {
                OpenAIContentItem::Text { text } => Some(text.clone()),
                OpenAIContentItem::ToolResult { content, is_error, .. } => {
...
impl ToolResultParser {
    ...
    pub fn parse_content(raw_content: &[u8]) -> Vec<ContentItem> {
        ...
        log(&format!("Using lossy UTF-8 conversion for tool result: {}", content_str));
    ...
    pub fn to_openai_message(message: Message) -> OpenAIMessage {
        ...
        for content_item in message.content {
            match content_item {
                ...
                MessageContent::ToolResult(ToolResult { tool_use_id, content: raw_content, is_error }) => {
        ...
        if let Some(openai_content) = openai_msg.content {
            for item in openai_content.items {
                match item {
                    ...
                    OpenAIContentItem::ToolResult { tool_use_id, content: items, is_error } => {
                        let serialized_content = serde_json::to_vec(&items)
                            .unwrap_or_else(|_| {
...
mod tests {
    ...
    fn test_content_format_serialization() {
        ...
        assert_eq!(string_format, serde_json::Value::String("Test message".to_string()));
